...
const UpdateProductForm = ({ params }: { params: any }) => {
  ...
  const [getProduct, { data: productData, loading: prodcutDataLoading }] =
  ...
  useEffect(() => {
    if (updateProductData?.updateProduct?.success) {
      ...
      !updateProductData?.updateProduct?.success
    ) {
      errorMessage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedCoverImage && typeof selectedCoverImage != "string") {
    ...
    <>
      <Breadcrumb
        items={[
          ...
          { label: "Edit product", value: "/admin/product/dfgsfgsdfgsdfgd" },
        ]}
      />
      <form
        className="max-w-full h-auto p-6 bg-white rounded shadow-md mt-4"
        onSubmit={handleSubmit}
      >
        <h2 className="text-md font-bold text-gray-500 mb-4">Edit Product</h2>
          ...
          <div className="w-5/6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div>
                  <select
                    value={formData.product_vip} // Set the selected value
                    name="product_vip"
                    onChange={handleChange}
